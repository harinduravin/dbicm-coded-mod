...
stddbicm = zeros(1,length(EbNoVec)); 
...
for ii = 1:m
    for n = 1:length(EbNoVec)
        ...
        llccapdelay = zeros(length(delaylabels),N);
        ...
        if delayscheme(ii) ~= 1
            for zz = 1:length(delaylabels)
                ...
                for j = 1:(N)
                    ...
                    cap(j) = (log2(prob(bitlevels(ii),0,y,EbNoVec(n),delaysymbollist,delaysymlabels)/prob(bitlevels(ii),ii,y,EbNoVec(n),delaysymbollist,delaysymlabels)));
                end
                llccapdelay(zz,:) = cap;
            end
            % Store the capacity
            capEst(ii,n) = 1-(1/length(delaylabels))*(1/N)*sum(llccapdelay,'All');
            stddbicm(n) = std(llccapdelay,0,'All');
        end
    end
end
...
figure
plot(EbNoVec,stddbicm/sqrt(N*length(delaylabels)/m),altstyles{8})
grid
legend('DBICM [1,0,1] std')
xlabel('Eb/No (dB)')
ylabel('Spectral Efficiency (bits/s/Hz)')
