...
capEst2 = zeros(m,length(EbNoVec));
...
hold on;
plot(EsNoVec,sum(capEst2,1),altstyles{8})
plot(EsNoVec,shannoncap,'-.')
plot(EsNoVec,ccCap,'-.')
plot(EsNoVec,sum(bicmCap,1),'-.')
grid
ylim([0 7])
legend('DBICM [0,0,1,0,0,1]','DBICM [1,0,1,1,0,1]','Shannon capacity','Const. Constr. capacity','BICM')
xlabel('Es/No (dB)')
