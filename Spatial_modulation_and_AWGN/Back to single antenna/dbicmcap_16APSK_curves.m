clear;
% EbNoVec = (-2:1:10)';      % Eb/No values (dB)
EsNoVec = (-2:1:10)'; 
N_bicm = 1000000;
...
figure
plot(EsNoVec,sum(capEst,1),altstyles{9})
hold on;
plot(EsNoVec,sum(dbicm2,1),altstyles{8})
plot(EsNoVec,shannoncap,'-.')
plot(EsNoVec,ccCap,'-.')
plot(EsNoVec,sum(bicmCap,1),'-.')
grid
ylim([0 4])
legend('DBICM [0,1,1,0]','DBICM [1,0,0,1]','log_2(1+SNR)','Const. Constr. capacity','BICM')
xlabel('Es/No (dB)')
ylabel('Spectral Efficiency (bits/s/Hz)')


speceff = linspace(1,3,10);
shannondb = 10*log10(2.^(speceff)-1);
bicmdb = interp1(sum(bicmCap,1),EsNoVec,speceff,"linear");
ccCapdb = interp1(ccCap,EsNoVec,speceff,"linear");
dbicmdb1 = interp1(sum(capEst,1),EsNoVec,speceff,"linear");
dbicmdb2 = interp1(sum(dbicm2,1),EsNoVec,speceff,"linear");
figure
...
grid
xlim([0.75 3.25])
legend('BICM','Const. Constr. capacity','DBICM [0,1,1,0]','DBICM [1,0,0,1]')
