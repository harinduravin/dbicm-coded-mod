...
% all_vectors = dec2bin(0:2^m-1, m) - '0';
% exclude_vectors = ~all(all_vectors == 0, 2) & ~all(all_vectors == 1, 2);
% valid_delay_vectors = all_vectors(exclude_vectors, :);
% 
% dbicmcaps = zeros(length(valid_delay_vectors),length(EbNoVec));
% 
% for v = 1:length(valid_delay_vectors)
% 
%     capEst = zeros(m,length(EbNoVec));
%     delayscheme = valid_delay_vectors(v,:);
% 
...
% for z = 1:length(valid_delay_vectors)
%     plot(EbNoVec,dbicmcaps(z,:),'-')
% end

grid
ylim([0 4])
legendentries = {'Shannon capacity',
    'Const. Constr. capacity',
    'BICM'
    };
% legend(cat(1,legendentries,cellstr(num2str(valid_delay_vectors, ' %d '))))
xlabel('Eb/No (dB)')
